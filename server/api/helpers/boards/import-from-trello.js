...
module.exports = {
  ...
  async fn(inputs) {
    ...
    const getAllTrelloCheckItemsOfCard = (cardId) => inputs.trelloBoard.checklists
      .filter((checklist) => checklist.idCard === cardId)
      .map((checklist) => checklist.checkItems)
      .flat();

    const getTrelloCommentsOfCard = (cardId) => inputs.trelloBoard.actions.filter(
    ...
    const importCardLabels = async (plankaCard, trelloCard) => Promise.all(
      trelloCard.labels.map(async (trelloLabel) => CardLabel.create({
    ...
    const importTasks = async (plankaCard, trelloCard) =>
      ...
      Promise.all(
        getAllTrelloCheckItemsOfCard(trelloCard.id).map(async (trelloCheckItem) => Task.create({
          cardId: plankaCard.id,
    ...
    const importComments = async (plankaCard, trelloCard) => {
      ...
      return Promise.all(
        trelloComments.map(async (trelloComment) => Action.create({
          cardId: plankaCard.id,
    ...
    const importCards = async (plankaList, trelloList) => Promise.all(
    ...
    const importLabels = async () => Promise.all(
    ...
    const importLists = async () => Promise.all(
